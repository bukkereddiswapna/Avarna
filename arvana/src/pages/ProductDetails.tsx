...
import { useProducts, isPurchasable, isVisibleToCustomers, getInventoryStatus } from "../context/ProductContext";
...
export default function ProductDetails() {
  ...
  if (!product || !isVisibleToCustomers(product)) return <Navigate to="/shop" replace />;

  const purchasable = isPurchasable(product);
  const inventoryStatus = getInventoryStatus(product);
  const related = products
    .filter((p) => p.id !== product.id && p.category === product.category && isVisibleToCustomers(p))
    .slice(0, 4);
      ...
      <div className="mx-auto max-w-[1440px] px-5 md:px-10">
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
          ...
          <div>
            ...
            <div className="mt-3 flex items-center gap-4">
              ...
              <span
                className={`text-xs uppercase tracking-wide ${
                  purchasable ? "text-forest" : "text-wood"
                }`}
              >
                {purchasable ? product.availability : "Out of Stock"}
              </span>
            </div>
            ...
            {purchasable && inventoryStatus === "LOW STOCK" && (
              <p className="mt-2 text-xs font-medium text-wood">Only {product.stock} left in stock — order soon.</p>
            )}
            ...
            <div className="mt-7 flex items-center gap-4">
              ...
              <div className="flex items-center border border-cream-border">
                ...
                <button
                  onClick={() =>
                    setQuantity((q) => (product.trackStock ? Math.min(product.stock, q + 1) : q + 1))
                  }
                  className="p-2.5 hover:bg-beige transition-colors"
