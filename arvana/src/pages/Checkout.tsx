...
export default function Checkout() {
  ...
  // Re-checked here (not just on the Cart page) in case stock changed in
  // the moments between leaving the cart and reaching checkout.
  ...
  const handlePlaceOrder = () => {
    ...
    setTimeout(() => {
      ...
      // Reduce stock for every purchased item and log it against this order,
      // so it's visible in Admin → Inventory → Stock History.
      orderItems.forEach((item) => {
        const product = getProductById(item.productId);
        if (product?.trackStock) {
          adjustStock(item.productId, -item.quantity, "Order", order.id);
        }
      });
      clearCart();
          ...
          <div className="h-fit border border-cream-border p-6 sm:p-8 bg-beige/30">
            ...
            {unavailableItems.length > 0 && (
              <p className="mt-2 text-center text-xs text-wood">
                ...
                <Link to="/cart" className="underline">Review cart</Link>
              </p>
            )}
          </div>
        </div>
